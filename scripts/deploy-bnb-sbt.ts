...
async function main() {
  ...
  // 获取合约工厂
  console.log("⏳ 正在编译合约...");
  const ZhengDaoSBT = await ethers.getContractFactory(contractName);
  ...
  if (deploymentTransaction) {
    const receipt = await ethers.provider.getTransactionReceipt(deploymentTransaction.hash);
    console.log("📊 部署交易详情:");
  ...
  console.log("\n2. 测试铸造 SBT:");
  console.log(`   npx hardhat run scripts/mint-test-sbt.ts --network bnbTestnet`);
  console.log("\n3. 更新 .env.local 文件:");
